...
1. Obsidian에서 **설정 → 커뮤니티 플러그인 → 탐색**을 열고 "CubicJ Brewing" 검색
2. 설치 후 활성화
3. 저울 연결 시 안내되는 원클릭 블루투스 애드온 다운로드를 진행하거나, **설정 → 블루투스 애드온**에서 설치
...
> zip에는 네이티브 블루투스 애드온(`noble/`)이 포함되어 있어 수동 설치는 완전히 오프라인으로 동작합니다. 폴더가 없어도 기록 기능은 그대로 동작하며, 저울 연결을 시도하면 동일한 체크섬 검증 다운로드가 안내됩니다.
