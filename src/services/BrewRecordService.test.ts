...
import type { BrewingRecord } from '../brew/types';
...
const makeBrewing = (overrides: Partial<BrewingRecord> = {}): BrewingRecord => ({
...
describe('BrewRecordService', () => {
	...
	it('getLastRecord returns most recent by bean x method', async () => {
		await service.add(makeBrewing({ timestamp: '2026-02-25T10:00:00Z', grindSize: 2.5 }));
		await service.add(makeBrewing({ timestamp: '2026-02-26T10:00:00Z', grindSize: 2.6 }));
		await service.add(makeBrewing({ bean: '룰 디카페인', timestamp: '2026-02-27T10:00:00Z' }));
		const last = await service.getLastRecord('첼로 블렌드', 'brewing');
