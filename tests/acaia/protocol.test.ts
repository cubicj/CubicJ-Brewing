...
describe('decodeSettings', () => {
	it('extracts battery, timerRunning, units, auto-off from Pearl S packet', () => {
		const payload = Buffer.from([
			0x1f, 0x64, 0x00, 0x02, 0x02, 0x03, 0x00, 0x00, 0x00, 0x01, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
			0x02,
		]);
	...
	it('detects timerRunning=true', () => {
		const payload = Buffer.from([0x1f, 0x64, 0x01, 0x02, 0x02, 0x03, 0x00, 0x00, 0x00, 0x01]);
		const result = decodeSettings(payload, 0);
...
describe('PacketBuffer', () => {
	...
	it('extracts multiple packets from single push', () => {
		...
		buf.push(Buffer.from([0xef, 0xdd, 0x0c, 0x01, 0x05, 0xaa, 0xef, 0xdd, 0x00, 0x02, 0x00, 0x02, 0x00]));
	...
	it('skips invalid payloadLen and recovers', () => {
		...
		buf.push(Buffer.from([0xef, 0xdd, 0xff, 0xff, 0x00, 0xef, 0xdd, 0x0c, 0x01, 0x05, 0xaa]));
		expect(packets.length).toBe(1);
		expect(packets[0][2]).toBe(0x0c);
	});
